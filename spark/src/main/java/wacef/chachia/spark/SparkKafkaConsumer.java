...
public class SparkKafkaConsumer {
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println("check position 1");
		
		JavaStreamingContext ssc = new JavaStreamingContext(sc, new Duration(2000)); //2 seconds
		System.out.println("check position 2");
		Map<String, String> kafkaParams = new HashMap<>();
		kafkaParams.put("metadata.broker.list", "localhost:9092");
		Set<String> topics = Collections.singleton("waceftopic");
		System.out.println("check position 3");
		JavaPairInputDStream<String, String> directKafkaStream = KafkaUtils.createDirectStream(ssc, String.class,
				String.class, StringDecoder.class, StringDecoder.class, kafkaParams, topics);

		List<String> allRecord = new ArrayList<String>();
		final String COMMA = ",";
		System.out.println("check position 4");
				...
				rdd.collect().forEach(rawRecord -> {
					...
					while (st.hasMoreTokens()) {
						...
						String isFraud = st.nextToken(); // dentifies a fraudulent transaction (1) and non fraudulent
															// (0)
						// String isFlaggedFraud = st.nextToken(); // flags illegal attempts to transfer
						// more than 200.000
